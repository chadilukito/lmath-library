...
uses
  Classes, Graphics, lmath.utypes, lmath.uErrors, lmath.umath, lmath.uround, 
  lmath.ustrings;
...
procedure WriteLegendSelect(Canvas     : TCanvas;
                            NSelect    : Integer;
                            Select     : TIntVector;
                            ShowPoints,
                            ShowLines  : Boolean);
...
begin
  ...
  { Allocate memory for curve parameters if necessary }
  if MaxCurv = 0 then
...
function AxisLabel(var Axis : TAxis; X : Float) : String;
begin
  if Axis.Scale = LinScale then
    AxisLabel := Trim(FloatStr(X))
  else if (X < -3.0) or (X > 3.0) then
    AxisLabel := '1.0E' + Trim(FloatStr(X))
  else
    AxisLabel := Trim(FloatStr(Exp10(X)));
end;
...
procedure SetGraphSettings(Canvas    : TCanvas; 
                           CurvIndex : Integer;
                           PlotType  : TPlotType); 
...
begin
  ...
  if PlotType = PltPoint then
    begin
      Canvas.Pen.Color := CurvParam[CurvIndex]^.PointParam.Color;
...
begin
  ...
  SetGraphSettings(Canvas, CurvIndex, PltPoint);
      ...
      if Flag2 then
        begin
          ...
          if (CurvParam[CurvIndex]^.LineParam.Style <> psClear) and Flag1 then
            begin
              SetGraphSettings(Canvas, CurvIndex, PltLine);
              PlotLine(Canvas, Xp1, Yp1, Xp2, Yp2);
            end  
        end;
...
begin
  ...
  for I := 1 to NSelect do
    begin
      ...
      if CurvParam[K]^.Legend <> '' then
        begin
          Inc(N);
          L := Canvas.TextWidth(CurvParam[K]^.Legend);
  ...
  for I := 1 to N do
    begin
      ...
      if ShowPoints then
        begin
          SetGraphSettings(Canvas, K, PltPoint);
          PlotSymbol(Canvas, Xp, Y, K);
        end;

      if ShowLines then
        begin
          SetGraphSettings(Canvas, K, PltLine);
          PlotLine(Canvas, Xp - L, Y, Xp + L, Y);
        end;
          
      { Write legend }
      Canvas.Brush.Style := bsClear;
      Canvas.TextOut(Xl, Y - CharHeight div 2, CurvParam[K]^.Legend);
    end;
...
var
  ...
  I      : Integer;
begin
  DimVector(Select, NCurv);
  for I := 1 to NCurv do
    Select[I] := I;
  WriteLegendSelect(Canvas, NCurv, Select, ShowPoints, ShowLines);
end;
