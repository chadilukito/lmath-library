...
{$IFDEF USE_DLL}
  dmath;
{$ELSE}  
  utypes, umath, utrigo, uhyper, 
  ugamma, udigamma, uigamma, ubeta, uibeta;
{$ENDIF}
...
function TimeToEval(N : Byte) : Integer;
...
begin
  ...
  TimeToEval := MilliSecondsBetween(Time, T0);
end;
...
begin
  ...
  Writeln;
  Writeln('Time in milliseconds to evaluate ', NMAX, ' functions');
  Writeln(F, 'Time in milliseconds to evaluate ', NMAX, ' functions');
  Writeln;
  Writeln(F);
  ...
  for N := 1 to NFUNC do
    begin
      T := TimeToEval(N);
      Writeln(FuncName[N], T:10);
      Writeln(F, FuncName[N], T:10);
    end;
