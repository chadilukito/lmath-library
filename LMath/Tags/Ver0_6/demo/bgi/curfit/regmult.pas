...
{$IFDEF USE_DLL}
  dmath;
{$ELSE}
  utypes, uScaling, umulfit, usvdfit,
...
procedure WriteResults(Y, Ycalc : TVector;
                       B        : TVector;
                       V        : TMatrix;
                       Test     : TRegTest;
                       Tc, Fc   : Float);
...
procedure PlotGraph(Y, Ycalc : TVector);
{ ------------------------------------------------------------------
...
var
  XX    : TMatrix;   { Independent variables }
  YY    : TVector;   { Dependent variable }
  Ycalc : TVector;   { Computed Y values }
  B     : TVector;   { Fitted parameters }
