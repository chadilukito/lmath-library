...
{$IFDEF USE_DLL}
  dmath, regutils;
{$ELSE}
  utypes, uErrors, uScaling, unlfit, ugamfit,
...
procedure SetBounds(var FirstPar, LastPar : Integer);
{ ------------------------------------------------------------------
...
begin
  ...
  { Perform regression }
  GammaFit(X, Y, 1, N, MaxIter, Tol, B, V);

  if MathErr <= MatOk then
    begin
      { Compute predicted Y values }
      for I := 1 to N do
        Ycalc[I] := GammaFit_Func(X[I], B);
