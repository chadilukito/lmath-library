{------------------------------------------------------------------------------
  ...
  Modified from Jean Debord's DMath uiexpfit. DMath may be obtained from:
  
  https://www.unilim.fr/pages_perso/jean.debord/tpmath/tpmath.htm 
  https://sourceforge.net/projects/dmath/ }

{ ******************************************************************
  This unit fits the decreasing exponential :
...
uses
  utypes, umath, umeansd, ulinfit, unlfit, UErrors;

procedure DecExpFit(X, Y     : TVector;
                    Lb, Ub   : Integer;
                    ConsTerm : Boolean;
                    MaxIter  : Integer;
                    Tol      : Float;
...
procedure WDecExpFit(X, Y, S  : TVector;
                     Lb, Ub   : Integer;
                     ConsTerm : Boolean;
                     MaxIter  : Integer;
                     Tol      : Float;
...
function DecExpFit_Func(X : Float; B : TVector) : Float;
{ ------------------------------------------------------------------
...
begin
  if gConsTerm then
    ...
    DecExpFit_Func := B[0] + B[1] * (Expo(- B[2] * X))
  else
    //IncExpFit_Func := B[1] * (1.0 - Expo(- B[2] * X));
    DecExpFit_Func := B[1] * (1.0 - Expo(- B[2] * X));
end;

procedure DecExpFit_Deriv(X, Y : Float; B, D : TVector);
{ --------------------------------------------------------------------
...
procedure GenDecExpFit(Mode     : TRegMode;
                       X, Y, S  : TVector;
                       Lb, Ub   : Integer;
                       ConsTerm : Boolean;
                       MaxIter  : Integer;
...
procedure DecExpFit(X, Y     : TVector;
                    ...
                    V        : TMatrix);
begin
  GenDecExpFit(OLS, X, Y, nil, Lb, Ub, ConsTerm, MaxIter, Tol, B, V);
end;
...
procedure WDecExpFit(X, Y, S  : TVector;
                     ...
                     V        : TMatrix);
begin
  GenDecExpFit(WLS, X, Y, S, Lb, Ub, ConsTerm, MaxIter, Tol, B, V);
end;
