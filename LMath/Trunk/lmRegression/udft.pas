...
Procedure FFTC1D(var A:TCompVector; N:Integer);
...
Array size N may be arbitrary number}
Procedure FFTC1DInv(var A:TCompVector; N:Integer);
...
Procedure FFTR1D(const A:TVector; N:Integer; var F:TCompVector);
...
Procedure FFTR1DInv(const F:TCompVector; N:Integer; var A:TVector);
...
Const
  ...
  //FTBaseCodeletMax:Integer = 5; //not used??
...
//The original Procedure uses "while loop" to do the "for loop" behavior. Rewrite it to "for loop".
var
  I,J,Idx1,Idx2,M2,M1,N1:Integer;
...
begin
  ...
  Move(Buf[0],A[AStart],(2*M*N)*SizeOf(Float)); //equivalent to original APVMove Procedure
end;
...
begin
  ...
  Move(Buf[0],A[AStart],(M*N)*SizeOf(Float)); //equivalent to original APVMove Procedure
end;
...
// during outer loop we update W[I,1].
  ...
  begin
    ...
    if I<(N2-1) then
    begin
      if (((I+1) mod (FTBaseUpdateTw*4))=0) then //(((I+1) mod 16)=0)
      begin
        V:=-2*Pi*(I+1)/N;
...
{Procedure FFTTwCalc(var A : TVector; AOffset,N1,N2:Integer); //ALGLIB 2.6
(*************************************************************************
Twiddle factors calculation
...
begin
  ...
  if I<(N2-1) then
  begin
    if (N1 mod FTBaseUpdateTw)=0 then
    begin
      V:=-2*Pi*(I+1)/N;
      TwRowXM1:=-2*IntPower(Sin(0.5*V),2);
      TwRowY:=Sin(V);
...
begin
  ...
  begin
    ...
    begin
      ...
      Move(Plan.TmpBuf[0],A[Offs],(2*N2*2)*SizeOf(Float)); //equivalent to original APVMove Procedure
    end;
...
*************************************************************************)
var
  I,{Idx,}N1,N2,N,M,Offs:Integer; //Idx is not used??
  V,BX,BY:Float;
//EmptyArray:TVector; //not used??
...
Procedure FTBaseExecutePlan(var A:TVector; AOffset{,N}:Integer; var Plan : TFTPlan); //N is not used??
(*************************************************************************
...
begin
  ...
  Move(A[0],Buf[0],N*SizeOf(Float)); //equivalent to original APVMove Procedure
...
begin
  ...
  begin
    SetLength(F,1);
    //F[0]:=Cmplx(A[0],0);
    F[0].X:=A[0];  //<---------- This is acually not needed. SetLength initializes to zero.
    F[0].Y:=0;     //<----------
    Exit;
  end;
...
begin
  ...
  if (N mod 2)=0 then
  begin
    H[(N div 2)]:=F[(N div 2)].X;
  end
  else
  begin
    H[(N div 2)]:=F[(N div 2)].X-F[(N div 2)].Y;
    H[(N div 2)+1]:=F[(N div 2)].X+F[(N div 2)].Y;
  end;
  FFTR1D(H,N,FH);
  for I := 0 to (N-1) do
  begin
